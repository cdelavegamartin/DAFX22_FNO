...
class TensionModulatedStringSolver:
    ...
    def create_random_initial(self, hi):
        ...
        y0 = hi * np.random.uniform(
            -1, 1, xx.shape
        )  # random initial condition -hi < y02 < hi

        yb0 = (
            X * sin(kmu[:, newaxis] * xx[newaxis, :]) @ y0
        )  # perform Fourier sine transformation as a Riemann sum
    ...
    def solve(self, wb0):
        ...
        yb1 = wb1[
            : self.M, :
        ].transpose()  # solution for the FS-transform of the deflection
