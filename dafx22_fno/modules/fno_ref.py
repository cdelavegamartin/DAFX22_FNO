...
class FNO_Markov_1d(torch.nn.Module):
    def __init__(self, in_channels, out_channels, spatial_size, width, depth=4):
        ...
        for _ in range(self.depth):
            self.fourier_conv_layers.append(
                SpectralConv1d(self.width, self.width, modes1=20)
            )
    ...
    def forward(self, x, num_time_steps):
        output = torch.zeros(
            x.shape[0], num_time_steps, self.spatial_size, self.out_channels
        ).to(x.device)
...
class FNO_Markov_2d(torch.nn.Module):
    def __init__(
        self, in_channels, out_channels, spatial_size_x, spatial_size_y, width, depth=4
    ):
        ...
        self.fc0 = nn.Linear(
            in_channels + 2, self.width
        )  # input channel is 3: (a(x, y), x, y)
        # The input gets concatenated with the grid values, this must also make a difference
        self.fc1 = nn.Linear(self.width, 128)
        self.fc2 = nn.Linear(128, out_channels)
        ...
        for _ in range(self.depth):
            self.fourier_conv_layers.append(
                SpectralConv2d(self.width, self.width, modes1=20, modes2=20)
            )
...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        out_ft[:, :, -self.modes1 :, : self.modes2] = self.compl_mul2d(
            torch.view_as_real(x_ft[:, :, -self.modes1 :, : self.modes2]), self.weights2
        )
