...
#######################################################################################################################
now = datetime.now()
directory = os.path.join("output", "1d_string_" + now.strftime("%H_%M_%S"))
...
cur_step = (
    output_sequence_gru.reshape(
        output_sequence_gru.shape[1],
        output_sequence_gru.shape[2] * output_sequence_gru.shape[3],
    )
    .detach()
    .cpu()
    .numpy()
...
cur_step = (
    output_sequence_rnn.reshape(
        output_sequence_rnn.shape[1],
        output_sequence_gru.shape[2] * output_sequence_gru.shape[3],
    )
    .detach()
    .cpu()
...
cur_step = (
    output_sequence_ref.reshape(
        output_sequence_ref.shape[1],
        output_sequence_gru.shape[2] * output_sequence_gru.shape[3],
    )
    .detach()
    .cpu()
...
gs = fig.add_gridspec(1, 4, hspace=0, wspace=0.05)
ax.scatter(
    (48000 / (2 * np.pi)) * np.angle(eigenvals_gru),
    20 * np.log10(np.abs(eigenvals_gru)),
    label="FGRU",
    s=9,
    marker="x",
    alpha=0.8,
...
ax.scatter(
    (48000 / (2 * np.pi)) * np.angle(eigenvals_rnn),
    20 * np.log10(np.abs(eigenvals_rnn)),
    label="FRNN",
    s=9,
    marker="*",
    alpha=0.8,
...
ax.scatter(
    (48000 / (2 * np.pi)) * np.angle(eigenvals_ref),
    20 * np.log10(np.abs(eigenvals_ref)),
    label="Ref.",
    s=9,
    c="grey",
    marker="v",
...
ax.scatter(
    (48000 / (2 * np.pi)) * np.angle(eigenvals_true),
    20 * np.log10(np.abs(eigenvals_true)),
    label="Truth",
    s=9,
    marker="s",
    c="purple",
